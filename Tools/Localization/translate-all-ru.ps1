...
$localeRoot = Join-Path $repoRoot "Resources\Locale\ru-RU"

if (-not (Test-Path $localeRoot)) {
    throw "Locale directory not found: $localeRoot"
}

$files = @(Get-ChildItem -Path $localeRoot -Recurse -Filter "*.ftl" -File |
    Sort-Object FullName |
    Select-Object -ExpandProperty FullName)

if ($files.Count -eq 0) {
    Write-Host "No .ftl files found in $localeRoot"
    exit 0
}
...
try {
    for ($index = 0; $index -lt $files.Count; $index += $BatchSize) {
        ...
        Write-Host "Translating all ru-RU files $($index + 1)-$($end + 1) of $($files.Count)..."
