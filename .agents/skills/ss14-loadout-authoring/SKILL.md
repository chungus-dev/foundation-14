...
2. `loadoutGroup`: `ScpLoadoutGroup` + slot name or generic group name.
...
3. Don't put copies in technical dumps if you can put them in a normal mirror path.
...
1. Leave the `_Scp` role to refer to the vanilla one or another fork's `loadoutGroup`.
