...
4. MUST: if the entity is unique to the fork and especially if it is a forked copy of the vanilla entity, add the fork prefix (`ScpXxx`).
...
5. Are you creating a fork-only or fork copy of vanilla?
   Add the `Scp` fork prefix to the ID (`Scp*`).
...
9. Fork ID with the prefix `Scp*` for unique fork content.
...
### Example 7: Scp prefix in fork content
