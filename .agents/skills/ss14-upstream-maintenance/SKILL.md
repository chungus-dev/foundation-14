...
This skill describes the standards and patterns adopted in this Space Station 14 fork with `_Scp` project-folder isolation for working with code inherited from the upstream.
...
After the active fork values are fixed, keep all new fork-owned code in `_Scp`. Do not switch folders because the touched behavior is vanilla, inherited from another fork, or looks "not fork-specific".
...
To clearly separate vanilla code from our modifications, the `_Scp` project folder is used.
...
The selected project folder is fixed for this repository, not chosen by the subsystem being touched. Write new project code under `_Scp`, even when the hook or parent prototype lives in vanilla code.
...
// SERVERNAME edit end
```
*Where `SERVERNAME` is `Scp`.*

**Example (change value):**
```csharp
component.Field2 = 321;
// Scp edit start - increasing radius for balance
component.Field = 123;
// Scp edit end
```
...
```csharp
// Scp edit start - fixing double gateways
...
**Example:**
```csharp
// Scp added start - to know when the shy one was hit
...
Instead, create a `partial` class in your project folder (`_Scp`).
...
2. Create a **new** YAML file in your project folder (for example, `Resources/Prototypes/_Scp/.../access.yml`).
...
*1. New prototype (`_Scp/Entities/Structures/Doors/Airlocks/access.yml`):*
```yaml
- type: entity
  parent: AirlockHatchSyndicate  # Inherit from the original
  id: AirlockHatchSyndicateLocked # New ID
  suffix: Syndicate, Locked
  categories: [ HideSpawnMenu ] # Scp added - hide from spawn if this is a technical entity
  components:
  - type: AccessReader
