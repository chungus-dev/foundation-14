...
public sealed class ScpRepairableSystem : EntitySystem
{
    ...
    private void OnInteractUsing(Entity<ScpRepairableComponent> ent, ref InteractUsingEvent args)
    {
        ...
        if (TryRepair(ent, args.User, args.Used, args.ClickLocation))
            args.Handled = true;
    }
    ...
    /// </summary>
    public bool TryRepair(Entity<ScpRepairableComponent> ent, EntityUid user, EntityUid used, EntityCoordinates clickLocation)
    {
        ...
        return Repair(ent, user, used, clickLocation);
    }
    ...
    /// </summary>
    private bool Repair(Entity<ScpRepairableComponent> ent, EntityUid user, EntityUid used, EntityCoordinates clickLocation)
    {
        if (!CanRepairStep(ent, used))
            return false;

        return DoRepairStep(ent, used, user, clickLocation);
    }
    ...
    private bool DoRepairStep(Entity<ScpRepairableComponent> ent, EntityUid used, EntityUid user, EntityCoordinates clickLocation)
    {
        ...
        {
            var edge = node.Edges[ent.Comp.EdgeIndex.Value];
            return DoRepairEdge(ent, used, user, clickLocation, edge, graph);
        }

        // Если мы не на ребре, выполняем нод
        return DoRepairNode(ent, used, user, clickLocation, node);
    }
    ...
    /// </summary>
    private bool DoRepairNode(Entity<ScpRepairableComponent> ent, EntityUid used, EntityUid user, EntityCoordinates clickLocation, ConstructionGraphNode node)
    {
        // Сбрасываем индекс шага
        ent.Comp.StepIndex = 0;
    ...
    /// </summary>
    private bool DoRepairEdge(Entity<ScpRepairableComponent> ent, EntityUid used, EntityUid user, EntityCoordinates clickLocation, ConstructionGraphEdge edge, ConstructionGraphPrototype graph)
    {
        if (!_construction.CheckConditions(ent, edge.Conditions))
            return false;
        ...
        if (!DoRepairStepAction(ent, used, user, clickLocation, step))
            return false;
    ...
    /// </summary>
    private bool DoRepairStepAction(Entity<ScpRepairableComponent> ent, EntityUid used, EntityUid user, EntityCoordinates clickLocation, ConstructionGraphStep step)
    {
        switch (step)
        {
            case EntityInsertConstructionGraphStep insertStep:
                ...
                var doAfterEv = CreateConstructionInteractDoAfter(ent, used, user, clickLocation);
            ...
            case ToolConstructionGraphStep toolInsertStep:
                var result = _tool.UseTool(
                    ...
                    [ toolInsertStep.Tool ],
                    CreateConstructionInteractDoAfter(ent, used, user, clickLocation),
                    out _,
                    toolInsertStep.Fuel);
    ...
    private ConstructionInteractDoAfterEvent CreateConstructionInteractDoAfter(
        EntityUid target,
        EntityUid used,
        EntityUid user,
        EntityCoordinates clickLocation)
    {
        var interactUsing = new InteractUsingEvent(user, used, target, clickLocation);
        return new ConstructionInteractDoAfterEvent(EntityManager, interactUsing);
    }
